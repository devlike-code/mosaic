...
use crate::internals::{
    slice_into_array, Bytesize, ComponentField, ComponentType, Datatype, DatatypeValue,
    EngineState, EntityId, S32,
};
...
impl<'a> BrickEditor<'a> {
    ...
    pub fn set_field(
        ...
        if flag {
            return Err("[Error][brick_editor.rs][set_field] Field datatype doesn't match with given datatype.".to_string());
        }
...
mod brick_editor_testing {
    ...
    fn test_read_complex_type_field_data() {
        ...
        let _ = engine_state
            .add_component_types("Position: product { x: f32, y: f64 };")
            .unwrap();
        ...
        if let Some(&brick_id) = query.as_slice().first() {
            let brick_editor = engine_state.get_brick_editor(brick_id).unwrap();
            {
                ...
                let offset = brick_editor
                    .get_field_offset(&component_type, "x".into())
                    .unwrap();
                assert_eq!(0, offset);
            ...
            }
            {
                ...
                let offset = brick_editor
                    .get_field_offset(&component_type, "y".into())
                    .unwrap();
                assert_eq!(4, offset);
                let field_value = &field_editor.data;

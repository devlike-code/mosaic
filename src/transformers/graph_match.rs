...
pub fn graph_match(input: &Tile, engine_state: Arc<MosaicEngine>) -> Result<Vec<Tile>, String> {
    let mut arrow_counters = usize::MAX;

    fn tuple_is_completely_different(a: &(usize, usize), b: &(usize, usize)) -> bool {
        a.0 != b.0 && a.1 != b.1
    }
    ...
    for child in pattern_children.into_iter() {
        ...
        for edge in child_edges {
            let (src, tgt) = edge.get_endpoints();
            pattern_graph.add_edge(edge.id(), src.id(), tgt.id());
        }
    }
    ...
    for child in target_children.into_iter() {
        ...
        for edge in child_edges {
            let (src, tgt) = edge.get_endpoints();
            target_graph.add_edge(edge.id(), src.id(), tgt.id());
        }
    }
    ...
    let mut pairing_to_index = HashMap::<((usize, usize), (usize, usize)), usize>::new();
    let mut index_to_pairing = HashMap::<usize, ((usize, usize), (usize, usize))>::new();
    ...
    for (s1t1, s2t2s) in &edge_graph.set {
        for s2t2 in s2t2s {
            ...
            let index = perpendicularity.adjacency.len() as usize;
            ...
            for s3t3 in s3t3s {
                if tuple_is_completely_different(&s1t1, &s3t3) {
                    ...
                    if (pairing_to_index.contains_key(&foreign_key1)
                        ...
                        && !perpendicularity.are_adjacent(index, second_index)
                    {
            ...
            if path.len() == pattern_size - 1 {
                ...
                if values.len() == pattern_size && bindings.len() == pattern_size {
                    ...
                    if !maps.contains(&h) {
                        ...
                        let _ = engine_state.set_parent(&binding_frame, &arrow);

                        for (k, v) in bindings {
                            if let (Some(k), Some(v)) =
                                (engine_state.get_tile(k), engine_state.get_tile(v))
                            {
                                let binding_arrow =
                                    engine_state.create_arrow(&k, &v, "Binding".into(), vec![])?;
                                let _ = engine_state.set_parent(&binding_arrow, &binding_frame);
                            }
                        }
                        maps.insert(h);
                    }
...
pub fn run_graph_match(
    graph_match_tile: &Tile,
    engine_state: Arc<MosaicEngine>,
) -> Result<GraphMatchHelper, String> {
    let res = graph_match(graph_match_tile, engine_state)?;

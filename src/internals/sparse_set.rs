...
impl SparseSet {
    ...
    pub(crate) fn get_pair_by_index(&self, sparse: EntityId) -> Option<DenseSparsePair> {
        if self.is_member(sparse) {
            ...
            assert!(index > 0);
            let dense = index - 1;
            Some(DenseSparsePair { dense_index: dense, sparse_index: sparse })
        } else {
            None
        }
    }
    ...
    pub fn get_index(&self, i: EntityId) -> Option<EntityId> {
        if self.is_member(i) {
            Some(self.index_array.get(&i).expect("Index not found (sparse_map:75)") - 1)
        } else {
            None
        }
    }
    ...
    pub fn is_member(&self, i: EntityId) -> bool {
        if self.order_max == 0 { return false; }

        if let Some(sp) = self.index_array.get(&i) {
            if *sp == 0 {
                return false;
            } else {
                let dn = *self.order_array.get((sp - 1) as usize).expect(format!("Dense map doesn't contain index {}", sp - 1).as_str());

...
pub fn generate_enum_code(enum_tile: &Tile) -> Result<String, (String, Tile)> {
    ...
    if let Some(name) = enum_tile.get_component("Label") {
        ...
        let spacing = if enum_tile.get_component("IndentWithSpaces").is_some() {
            "  "
        } else {
            "\t"
        };
...
mod primitive_code_gen_tests {
    ...
    fn test_enums() {
        ...
        mosaic.new_type("IndentWithSpaces: unit;").unwrap();
        ...
        mosaic.new_descriptor(&e, "IndentWithSpaces", void());
